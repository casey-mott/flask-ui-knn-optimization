...
def knn(label_pick, split_size, filename):

    test_size = int(split_size) / 100

    split_data(filename, label_pick, test_size)
    ...
    ### Plot unsorted df as line graph

    score_df = pd.DataFrame(score_list)

    save_csv_in_memory(score_df, 'data/scores_to_plot.csv')

    plot_line_graph('data/scores_to_plot.csv', split_size)

    os.remove('data/scores_to_plot.csv')

    score_df = score_df.sort_values(by=['accuracy', 'k'], ascending=[False, True])

    best = score_df['k'].to_list()[0]
    accuracy = score_df['accuracy'].to_list()[0]

    return best, accuracy


def plot_line_graph(filename, split_size):

    df = pd.read_csv(filename)

    ax = df.plot.line(x='k', y='accuracy')
    ax.set_title('Accuracy by Neighbor - ' + str(split_size) + '% Train')
    plt.tight_layout()

    png_name = './static/line_' + str(split_size) + '.png'
